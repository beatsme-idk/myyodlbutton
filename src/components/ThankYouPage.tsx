...
const ThankYouPage = ({ thankYou }: ThankYouProps) => {
  ...
  // Default animation classes based on the animation type
  const getAnimationClass = () => {
    switch (thankYou.animation) {
      case "bounce":
  ...
  // Use accent color or text color as fallback
  const accentColor = thankYou.accentColor || thankYou.textColor;
  ...
  return (
    ...
    >
      ...
      <div className="max-w-md w-full text-center space-y-8 relative z-10 animate-fade-in">
        ...
        <Heart className="mx-auto" size={60} style={{ color: accentColor }} />
        
        <h1 className="text-4xl font-bold tracking-tight animate-slide-up">
          {thankYou.headerText || "Thank You!"}
        </h1>
        ...
        {/* Transaction details */}
        {(thankYou.showTransactionDetails === undefined || thankYou.showTransactionDetails) && (
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 justify-center animate-slide-up delay-450">
          {(thankYou.showReturnHomeButton === undefined || thankYou.showReturnHomeButton) && (
            <Button 
              onClick={() => navigate("/")}
              variant="outline"
          ...
          {(thankYou.showShareButton === undefined || thankYou.showShareButton) && (
            <Button
